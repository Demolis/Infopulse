...
#about factor
mas<-rnorm(100,10,6)
cut(mas,breaks=c(min(mas),mean(mas)-2*sd(mas),mean(mas)+2*sd(mas), max(mas)), labels = c("First","Second","Third"),include.lowest = T)
#Slide 2
#1
df1<-read.csv("DataDay4a.csv",sep=";")
df2<-read.csv("DataDay4b.csv",sep=";")
str(df1)
#2
...
#3
cor(df[,4:11],use = "pairwise.complete.obs")
